...
const SET_SERVER_ANSWER = 'booking/booking/SET_SERVER_ANSWER';

const booking_reducer = (state = initialState, action: any): InitialStateType => {
...
export const setServerAnswer = (serverAnswer: any) => {
    return {type: SET_SERVER_ANSWER, serverAnswer: serverAnswer};
}
...
export const uploadBooking = (bookingData: any) => async (dispatch: any) => {
    try{
        const result = await API.uploadBooking(bookingData);
        if(result !== undefined){
            // dispatch(setBooking(result.booking));
            // dispatch(setCount(result.count));
            dispatch(setServerAnswer(result));
        }
    }catch(error){
        console.log(error)
        // dispatch(setNoInet(true))
